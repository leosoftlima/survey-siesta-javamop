...
HEATMAP_PNG = os.path.join(FIGS_DIR, "rq1_experience_heatmap_javamop.png")
HEATMAP_PDF = os.path.join(FIGS_DIR, "rq1_experience_heatmap_javamop.pdf")
LATEX_VARS_TEX = os.path.join(VARS_DIR, "rq1_experience_vars_javamop.tex")
...
# DADOS
# =========================
def compute_heatmap_data_javamop(df, exp_col):
    ...
    for exp in experience_levels:
        ...
        for scenario in range(1, 12):
            ...
            if col in filtered.columns:
                ...
                total = len(votes)
                javamop_count = (votes == "JavaMOP").sum()
                percent = (javamop_count / total) * 100 if total > 0 else 0
...
def export_latex_variables_rq1_javamop(heatmap_df):
    vars_out = {}

    # Médias por experiência (JavaMOP)
    for exp in heatmap_df.index:
        suffix = sanitize_exp_label(exp)
        avg = round(float(heatmap_df.loc[exp].mean()), 1)
        vars_out["percJavaMOP" + suffix] = avg
    ...
    vars_out["scenarioJavaMOPMaxScenario"] = max_scenario
    vars_out["scenarioJavaMOPMaxPercent"] = max_percent
    vars_out["scenarioJavaMOPMinScenario"] = min_scenario
    vars_out["scenarioJavaMOPMinPercent"] = min_percent
    vars_out["scenarioClosestScenarioJavaMOP"] = closest_scenario
    vars_out["scenarioClosestPercentJavaMOP"] = closest_percent
...
# PLOT
# =========================
def plot_experience_heatmap_javamop(heatmap_df, save_png, save_pdf=None):
    sns.set_style("white")
...
def main():
    ...
    heatmap_javamop = compute_heatmap_data_javamop(df, exp_col)

    plot_experience_heatmap_javamop(
        heatmap_javamop,
        HEATMAP_PNG,
        HEATMAP_PDF
    )

    latex_vars = export_latex_variables_rq1_javamop(heatmap_javamop)
